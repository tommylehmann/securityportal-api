...
func TestMigrateAppliesIncrementalOnUpgrade(t *testing.T) {
	...
	// Simulate a legacy install: an older base schema recorded at version 0 (the
	// pre-fix scheme) that predates the incremental objects. The incrementals
	// (001 ingest-state/tombstone, 002 facets+FTS) are written idempotently
	// against this. The fixture mirrors the slice of the base schema the
	// incrementals touch: advisories plus a minimal documents table (002 adds
	// FKs, the tsv column and triggers to documents, and its backfill reads the
	// generated lang column).
	const legacySchema = `
		...
		CREATE TABLE documents (
			id            int PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
			advisories_id int NOT NULL REFERENCES advisories(id) ON DELETE CASCADE,
			lang          text GENERATED ALWAYS AS (document #>> '{document,lang}') STORED,
	...
	// This must apply the incrementals (and only the incrementals) on top of the
	// version-0 install.
	if err := Migrate(ctx, pool); err != nil {
	...
	// versions now holds the 0 row plus one row per applied incremental — and the
	// re-apply did not add duplicates. The incrementals did NOT record version 0
	// again (no collision).
	...
	if len(versions) != 3 || versions[0] != 0 || versions[1] != 1 || versions[2] != 2 {
		t.Fatalf("versions after upgrade = %v, want [0 1 2]", versions)
	}
...
// TestMigrate002BackfillsPreExistingRows pins the task-15/16 backfill: a
// database created from an EARLIER base schema (recorded at version 0) that
// predates the facet/FTS objects, holding a document inserted before those
// objects existed, must — on the next Migrate — gain its documents_cves,
// documents_products and tsv from the 002 backfill, without re-ingestion. A
// second Migrate is a no-op (the migration already ran) and must not duplicate
// the backfilled rows.
func TestMigrate002BackfillsPreExistingRows(t *testing.T) {
	pool, ctx := startPostgres(t)

	// A legacy base recorded at version 0, before the facet/FTS objects. It
	// carries only the slice of the base schema the backfill reads: a documents
	// table with the generated lang column and the JSONB. No facet tables, no tsv
	// column, no extraction triggers exist yet, so the pre-existing row is
	// un-faceted until 002's backfill runs.
	const legacyBase = `
		CREATE TABLE versions (
			version     int PRIMARY KEY,
			description text NOT NULL,
	...
	if _, err := pool.Exec(ctx, legacyBase); err != nil {
		t.Fatalf("seeding legacy base: %v", err)
	}

	// Insert a pre-existing document (no extraction triggers fire — they do not
	// exist yet). It has two CVEs, a nested product_tree, and a German title.
	const preExisting = `{
		"document": {
			"title": "Mehrere Schwachstellen in ExampleApp",
			"lang": "de-DE",
			"category": "csaf_security_advisory",
			"publisher": {"name": "SecurityPortal Test Publisher"}
		},
		"product_tree": {
			"branches": [
				{"category": "vendor", "name": "Example AG", "branches": [
	...
	var advID int
	if err := pool.QueryRow(ctx,
		`INSERT INTO advisories (tracking_id, publisher) VALUES ('LEGACY-1', 'SecurityPortal Test Publisher') RETURNING id`,
	).Scan(&advID); err != nil {
		t.Fatalf("seeding legacy advisory: %v", err)
	}
	var docID int
	if err := pool.QueryRow(ctx,
		`INSERT INTO documents (advisories_id, document) VALUES ($1, $2::jsonb) RETURNING id`,
	...
	// Run the incrementals. 002 creates the facet/FTS objects and backfills them
	// for the already-present row.
	if err := Migrate(ctx, pool); err != nil {
		t.Fatalf("Migrate upgrading the legacy DB: %v", err)
	}

	// The pre-existing row gained both CVEs.
	var cveCount int
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM documents_cves WHERE documents_id = $1`, docID).Scan(&cveCount); err != nil {
		t.Fatalf("counting backfilled CVEs: %v", err)
	}
	if cveCount != 2 {
		t.Errorf("backfill gave %d CVE rows, want 2", cveCount)
	}

	// And the (vendor, product) pairs: bare vendor + the product under it.
	var prodCount int
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM documents_products WHERE documents_id = $1`, docID).Scan(&prodCount); err != nil {
		t.Fatalf("counting backfilled products: %v", err)
	}
	if prodCount != 2 {
		t.Errorf("backfill gave %d product rows, want 2 (vendor + product)", prodCount)
	}

	// And a language-correct, non-empty tsvector (german config => stem match).
	var tsvNonEmpty, germanMatch bool
	if err := pool.QueryRow(ctx, `
		SELECT tsv IS NOT NULL AND tsv <> ''::tsvector,
		       tsv @@ plainto_tsquery('german', 'Schwachstelle')
		FROM documents WHERE id = $1`, docID).Scan(&tsvNonEmpty, &germanMatch); err != nil {
		t.Fatalf("reading backfilled tsv: %v", err)
	}
	if !tsvNonEmpty {
		t.Error("backfill left tsv empty for the pre-existing row")
	}
	if !germanMatch {
		t.Error("backfilled tsv should be german-stemmed (matched by 'Schwachstelle')")
	}

	// Idempotency: a second Migrate is a no-op (002 already recorded) and must
	// not duplicate the backfilled facet rows.
	if err := Migrate(ctx, pool); err != nil {
		t.Fatalf("second (idempotent) Migrate: %v", err)
	}
	var cveAfter, prodAfter int
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM documents_cves WHERE documents_id = $1`, docID).Scan(&cveAfter); err != nil {
		t.Fatalf("re-counting CVEs: %v", err)
	}
	if err := pool.QueryRow(ctx,
		`SELECT count(*) FROM documents_products WHERE documents_id = $1`, docID).Scan(&prodAfter); err != nil {
		t.Fatalf("re-counting products: %v", err)
	}
	if cveAfter != 2 || prodAfter != 2 {
		t.Errorf("re-running Migrate duplicated facet rows: cves=%d products=%d, want 2 and 2",
			cveAfter, prodAfter)
	}
}
